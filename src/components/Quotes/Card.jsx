...
export default function Card({cardInfo,i}) {
   ...
   //data is recieved as props and displayed in the card 
    return (
        <div style={ cardContainer2} className='justify-items-center flex flex-col items-center  sm:flex-row-reverse '>
            <div className=' flex-1 pt-8  rounded-l-lg   ' >
                <img src={cardInfo.img} alt='card ' className='  rounded-l-3xl rounded-r-3xl'/>
            </div>
            <div className='items-center flex flex-col items-center  flex-1 sm:items-start  '>
                ...
                <h1 className='text-blue-500 sm:text-base md:text-base self-center sm:self-start xl:text-2xl lg:text-2xl   flex-1 items-start bold font-bold '>{cardInfo.head}</h1>
                
                <p className='flex-1 sm:left text-center sm:text-left pt-4 leading-loose lg:text-base xl:text-base md:text-sm sm:text-sm text-sm  w-4/5 xl:w-3/4	'>{cardInfo.para}</p>
            </div>
        </div>
    )
}
